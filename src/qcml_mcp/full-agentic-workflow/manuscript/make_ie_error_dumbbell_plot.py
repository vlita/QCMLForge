"""Generate dumbbell plots comparing actual and predicted IE errors."""
    ...
    for ax, system in zip(axes, SYSTEM_ORDER):
        ...
        for tick in ax.get_xticks():
            if x_min <= tick <= x_max and abs(tick) > 1e-9:
                ax.axvline(tick, color=GRID_LINE_COLOR, lw=GRID_LINE_WIDTH, zorder=0)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-prefix",
        type=Path,
        default=Path(__file__).with_name("ie_error_dumbbell_plot"),
